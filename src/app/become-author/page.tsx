...
export default function BecomeAuthor() {
  ...
  useEffect(() => {
    ...
    if ((session?.user as any)?.role === "author" || (session?.user as any)?.role === "admin") {
  ...
  if (status === "loading") {
    return (
      <div style={{ minHeight: "100vh", background: "#080808", display: "flex", alignItems: "center", justifyContent: "center", color: "#f0e6d0" }}>
        Đang tải...
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: "#080808", color: "#f0e6d0" }}>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(8,8,8,0.95)", borderBottom: "1px solid rgba(201,168,76,0.1)", backdropFilter: "blur(20px)", padding: "0 40px", display: "flex", alignItems: "center", justifyContent: "space-between", height: "64px" }}>
        <div onClick={() => router.push("/")} style={{ display: "flex", alignItems: "center", gap: "12px", cursor: "pointer" }}>
          ...
          <div style={{ fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: 18, letterSpacing: "0.1em" }}>
            M<span style={{ color: "#c9a84c" }}>AI</span>NGA
          </div>
        </div>
        <button onClick={() => router.push("/profile")} style={{ background: "transparent", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "6px", padding: "8px 18px", color: "rgba(240,230,208,0.5)", fontFamily: "'Inter', sans-serif", fontSize: 13, cursor: "pointer" }}>
          ← Hồ sơ
        </button>
      </nav>
      ...
      {step < 4 && (
        <div style={{ textAlign: "center", padding: "60px 24px 48px", background: "linear-gradient(180deg, rgba(201,168,76,0.04) 0%, transparent 100%)" }}>
          <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, letterSpacing: "0.4em", color: "#c9a84c", textTransform: "uppercase", marginBottom: 20 }}>
            ✦ Dành Cho Người Sáng Tạo ✦
          </div>
          <h1 style={{ fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: "clamp(32px, 5vw, 56px)", lineHeight: 1.1, marginBottom: 16 }}>
            Trở Thành
            <br />
          ...
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 15, color: "rgba(240,230,208,0.4)", maxWidth: 500, margin: "0 auto", lineHeight: 1.8 }}>
            Chia sẻ câu chuyện của bạn với hàng nghìn độc giả.
          </p>
        </div>
      )}
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "0 24px 80px" }}>
        {step < 4 && (
          <div style={{ marginBottom: 40 }}>
            ...
            <div style={{ height: 2, background: "rgba(255,255,255,0.05)", borderRadius: 1 }}>
              <div style={{ height: "100%", width: `${((step - 1) / 2) * 100}%`, background: "linear-gradient(90deg, #c9a84c, #8b6914)", transition: "width 0.4s ease" }} />
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="fade-up" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "16px", padding: "40px" }}>
            ...
            <button className="gold-btn" onClick={() => setStep(2)} style={{ width: "100%", padding: "14px", borderRadius: "8px", color: "#080808", fontFamily: "'Inter', sans-serif", fontSize: 14, fontWeight: 700, marginTop: 24, letterSpacing: "0.1em" }}>
              TIẾP THEO →
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="fade-up" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "16px", padding: "40px" }}>
              ...
              {GENRES.map((g) => (
                <button
                  ...
                  style={{
                    padding: "10px 18px",
                    borderRadius: "30px",
                    background: form.genre.includes(g) ? "linear-gradient(135deg, #c9a84c, #8b6914)" : "rgba(255,255,255,0.03)",
                    border: form.genre.includes(g) ? "none" : "1px solid rgba(201,168,76,0.15)",
                    color: form.genre.includes(g) ? "#080808" : "rgba(240,230,208,0.45)",
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              <button onClick={() => setStep(1)} style={{ flex: 1, padding: "13px", background: "transparent", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "8px", color: "rgba(240,230,208,0.4)", fontFamily: "'Inter', sans-serif", fontSize: 13, cursor: "pointer" }}>
                ← QUAY LẠI
              </button>
              <button className="gold-btn" onClick={() => setStep(3)} style={{ flex: 2, padding: "13px", borderRadius: "8px", color: "#080808", fontFamily: "'Inter', sans-serif", fontSize: 14, fontWeight: 700, letterSpacing: "0.1em" }}>
                TIẾP THEO →
              </button>
            </div>
          </div>
        ...
        {step === 3 && (
          <div className="fade-up" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "16px", padding: "40px" }}>
            <h2 style={{ fontFamily: "'Cinzel', serif", fontSize: 22, marginBottom: 24, letterSpacing: "0.08em" }}>Xác Nhận</h2>
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 14, color: "rgba(240,230,208,0.5)", lineHeight: 1.8, marginBottom: 24 }}>
              Sau khi xác nhận, tài khoản hiện tại của bạn sẽ được nâng lên thành <b style={{ color: "#c9a84c" }}>author</b>.
            </div>
            {msg && <div style={{ color: "#ff6b6b", fontFamily: "'Inter', sans-serif", fontSize: 13, marginBottom: 16 }}>{msg}</div>}
            <div style={{ display: "flex", gap: "12px" }}>
              <button onClick={() => setStep(2)} style={{ flex: 1, padding: "13px", background: "transparent", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "8px", color: "rgba(240,230,208,0.4)", fontFamily: "'Inter', sans-serif", fontSize: 13, cursor: "pointer" }}>
                ← QUAY LẠI
              </button>
              <button className="gold-btn" onClick={handleSubmit} disabled={loading} style={{ flex: 2, padding: "13px", borderRadius: "8px", color: "#080808", fontFamily: "'Inter', sans-serif", fontSize: 14, fontWeight: 700, letterSpacing: "0.1em", opacity: loading ? 0.7 : 1 }}>
                {loading ? "ĐANG XỬ LÝ..." : "✦ TRỞ THÀNH TÁC GIẢ"}
              </button>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="fade-up" style={{ textAlign: "center", padding: "60px 40px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "16px" }}>
            ...
            <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 15, color: "rgba(240,230,208,0.5)", lineHeight: 1.8, marginBottom: 40 }}>
              Tài khoản của bạn đã được nâng cấp thành tác giả.
            </p>
            <button className="gold-btn" onClick={() => router.push("/dashboard")} style={{ padding: "14px 32px", borderRadius: "8px", color: "#080808", fontFamily: "'Inter', sans-serif", fontSize: 14, fontWeight: 700, letterSpacing: "0.1em" }}>
              ✦ VÀO DASHBOARD
            </button>
          </div>
        )}
