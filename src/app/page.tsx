  ...
  async function handleUploadManga() {
    ...
    try {
      ...
      if (chapterRes.ok) {
        ...
        setCoverPreview("");
        setUploadedPages([]);
        setUploadForm({ title: "", description: "", genre: [] });
        setChapterNum(1);
        setChapterTitle("");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", color: "#f0e6d0", fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "100px 40px 80px", textAlign: "center", overflow: "hidden" }}>
        ...
        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, letterSpacing: "0.4em", color: "#c9a84c", textTransform: "uppercase", marginBottom: 24 }}>✦ Nền Tảng Đọc Và Sáng Tác Mainga Hàng Đầu ✦</div>
        ...
        {/* SEARCH */}
        <div style={{ display: "flex", alignItems: "center", maxWidth: 560, margin: "0 auto 64px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "40px", overflow: "hidden", boxShadow: "0 8px 32px rgba(0,0,0,0.3)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "64px", flexWrap: "wrap" }}>
          {[["12,400+", "Bộ Manga", "📚"], ["890K+", "Lượt Đọc", "👁"], ["34K+", "Tác Giả", "✍"]].map(([num, label, icon]) => (
            <div key={label} style={{ textAlign: "center" }}>
              <div style={{ fontFamily: "'Cinzel', serif", fontSize: "clamp(24px, 3vw, 36px)", color: "#c9a84c", fontWeight: 600 }}>{num}</div>
                ...
                <div style={{ position: "relative", borderRadius: "10px", overflow: "hidden", aspectRatio: "3/4", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(201,168,76,0.1)", marginBottom: 12 }}>
                  ...
                  {hoveredManga === manga.id && (
                    <div style={{ position: "absolute", inset: 0, background: "rgba(201,168,76,0.08)", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(2px)" }}>
                      <div style={{ padding: "10px 22px", background: "linear-gradient(135deg, #c9a84c, #8b6914)", borderRadius: "6px", fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 600, color: "#080808" }}>Đọc Ngay</div>
                    </div>
                  )}
                </div>
      ...
      {showAuth && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowAuth(false)}>
          <div className="modal-box glass-card" style={{ borderRadius: "16px", padding: "48px", width: "100%", maxWidth: 420, border: "1px solid rgba(201,168,76,0.2)", boxShadow: "0 32px 80px rgba(0,0,0,0.8)" }}>
            <div style={{ textAlign: "center", marginBottom: 36 }}>
              <div style={{ width: 52, height: 52, background: "linear-gradient(135deg, #c9a84c, #8b6914)", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Cinzel', serif", fontWeight: 700, fontSize: 22, color: "#080808", margin: "0 auto 20px" }}>墨</div>
            ...
            <button className="gold-btn" onClick={handleAuth} disabled={loading} style={{ width: "100%", padding: "14px", borderRadius: "8px", color: "#080808", fontFamily: "'Inter', sans-serif", fontSize: 14, fontWeight: 700, marginBottom: 20, letterSpacing: "0.1em" }}>
          ...
          <div className="modal-box glass-card" style={{ borderRadius: "16px", padding: "40px", width: "100%", maxWidth: 540, maxHeight: "90vh", overflowY: "auto", border: "1px solid rgba(201,168,76,0.2)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                ...
                <div
                  ...
                  onDrop={e => {
                    ...
                    if (file && file.type.startsWith("image/")) { setCoverImage(file); setCoverPreview(URL.createObjectURL(file)); }
                  }}
                  onClick={() => document.getElementById('cover-upload')?.click()}
                  style={{ border: `1px dashed ${dragOver ? "#c9a84c" : coverPreview ? "#c9a84c" : "rgba(201,168,76,0.2)"}`, borderRadius: "8px", padding: coverPreview ? "8px" : "32px", textAlign: "center", cursor: "pointer", transition: "all 0.3s", background: dragOver ? "rgba(201,168,76,0.05)" : "transparent" }}
                >
                  {coverPreview ? (
            ...
            {uploadStep === 2 && (
              <div>
                <div style={{ display: "flex", gap: "12px", marginBottom: 16 }}>
                  <input style={{ flex: 1, padding: "12px 16px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "8px", color: "#f0e6d0", fontFamily: "'Inter', sans-serif", fontSize: 14, outline: "none" }} placeholder="Số chapter *" type="number" min={1} value={chapterNum} onChange={e => setChapterNum(parseInt(e.target.value) || 1)} />
                  <input style={{ flex: 2, padding: "12px 16px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: "8px", color: "#f0e6d0", fontFamily: "'Inter', sans-serif", fontSize: 14, outline: "none" }} placeholder="Tên chapter (VD: Khởi Đầu)" value={chapterTitle} onChange={e => setChapterTitle(e.target.value)} />
                </div>
                <div
                  onDragOver={e => { e.preventDefault(); setDragOver(true); }}
                  onDragLeave={() => setDragOver(false)}
              ...
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                {uploading ? (
                  ...
                  <>
                    ...
                    <div style={{ fontFamily: "'Cinzel', serif", fontSize: 16, color: "#c9a84c", marginBottom: 12 }}>{uploadForm.title}</div>
                    <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, color: "rgba(240,230,208,0.4)", marginBottom: 8 }}>Chapter {chapterNum} · {uploadedPages.length} trang</div>
                    <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, color: "rgba(240,230,208,0.6)", lineHeight: 1.7 }}>Manga của bạn sẵn sàng được đăng lên thế giới!</div>
                  </>
                )}
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: "12px", marginTop: 32 }}>
              {uploadStep > 1 && !uploading && (
                <button onClick={() => setUploadStep(s => s - 1)} style={{ flex: 1, padding: "13px", borderRadius: "8px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(201,168,76,0.15)", color: "rgba(240,230,208,0.5)", fontFamily: "'Inter', sans-serif", fontSize: 13, cursor: "pointer", letterSpacing: "0.08em" }}>← QUAY LẠI</button>
              )}
